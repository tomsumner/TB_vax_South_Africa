...
suppressPackageStartupMessages({
  ...
  source(here("R", "workflow", "run_param_set_econ_SAedt.R"))
...
if (T){ # on local machine
  cc <- "ZAF"
  vx_scenarios <- "econ_vx_scenarios_all.csv"#econ_vx_scenarios_2501.csv
  econ = T
...
parameters <- fread(here("./processing_files/paramsets/paramsets.csv"))
...
for (i in 1:nrow(vx_scenarios)){ #
  ...
  # subset to the i-th row of characteristics
    ...
    if (vx_chars$runtype != "baseline") {
      ...
      if (grepl("repeats", vx_chars$runtype) == TRUE) { 
        
        vx_inc <- c(vx_chars$prev, vx_chars$vac, vx_chars$prevB, vx_chars$vacB)
    ...
    vx_scen_output <- run_param_set_econ(model, cc, params, params_uid,
                                          vx_chars, vx_inc, rr_pct = rr_prop)
