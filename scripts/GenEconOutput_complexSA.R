...
suppressPackageStartupMessages({
  ...
  source(here("R", "workflow", "run_param_set_econ_SA.R"))
})
...
if (T){ # on local machine
  cc <- "ZAF"
  vx_scenarios <- "econ_vx_scenarios_M72.csv"
  econ = T
  taskenvvar="something"
} else{ # on cluster
...
#econ_check <- open_dataset(sources = "./econ_output/ZAF_TB/") %>% dplyr::collect()
