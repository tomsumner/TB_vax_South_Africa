...
##orignal xml to run epi output with the new L0 structure for calibration
#filepath <- "countries/ZAF/parameters/"
input    <- read_xml("countries/ZAF/parameters/XMLinput_target_L0.xml")

op  <- xml_find_all(input, xpath='//output')

filename <- c("XMLinput_target_L0_run.xml")

#create xml for generating epi output
...
xml_set_attr(x=xml_find_all(x = op, xpath='detailed.output[@years = "c(2000:2021)+0.5"]'),
             attr = "years", value = "c(2000:2050)+0.5")

xml_set_attr(x=xml_find_all(xpath='//simulation'))
             
xml_find_all(xpath='/simulation')

write_xml(input, file = paste0("countries/ZAF/parameters/",filename[1])) 


###

#create xml for generating econ output
#create xml for ltbi by age
