...
relative_output <- fread("./epi_output/grouped_output/relative_output.csv")
relative_output <- relative_output[, .(Year, AgeGrp, Runtype, Indicator, medval, lowval, highval)]

raw_output <- fread("./epi_output/grouped_output/raw_output.csv")
...
# Basecase only
if (F){
  pdf("M72_rr.pdf",width = 12, height=7.5)
ggplot(epi_M72_2050[Runtype == "Basecase" & Indicator %in% rate_reductions]) +
  labs(title=(expression(paste(M72/AS01[E],
                               "-like Basecase vaccine scenario: Rate reductions in 2050")))) +
...
pdf("M72_averted.pdf",width = 12, height=7.5)
ggplot(epi_M72_2050[Indicator %in% averted_numbers & Runtype == "Basecase"]) +
  labs(title=(expression(paste(M72/AS01[E],
                               "-like Basecase vaccine scenario: Averted numbers by 2050")))) +
  geom_col(aes(x = Indicator, y = medval, fill = Runtype),
           position = position_dodge(), alpha = 0.5, width = 0.5) +
  geom_errorbar(aes(x = Indicator, ymin = lowval, ymax = highval, col = Runtype),
                position = position_dodge(width = .5), size = 1, width = 0.25) +
...
if (F){
  pdf("M72_rr_all.pdf",width = 12, height=7.5)
  ...
  pdf("M72_averted_all.pdf",width = 12, height=7.5)
...
# Basecase only
if (F){
  pdf("BCG_rr_bc.pdf", width=12, height=7.5)
  ggplot(epi_BCG_2050[Indicator %in% rate_reductions & Runtype == "Basecase"]) +
    labs(title="BCG-revaccination-like Basecase vaccine scenario: Rate reductions in 2050") +
    geom_col(aes(x = Indicator, y = medval, fill = Runtype),
             position = position_dodge(), alpha = 0.5, width = 0.5) +
    geom_errorbar(aes(x = Indicator, ymin = lowval, ymax = highval, col = Runtype),
                  position = position_dodge(width = .5), size = 1, width = 0.25) +
  ...
  pdf("BCG_averted_bc.pdf", width = 12, height = 7.5)
  ggplot(epi_BCG_2050[Indicator %in% averted_numbers & Runtype == "Basecase"]) +
    labs(title="BCG-revaccination-like Basecase vaccine scenario: Averted numbers by 2050") +
    geom_col(aes(x = Indicator, y = medval, fill = Runtype),
             position = position_dodge(), alpha = 0.5, width = 0.5) +
    geom_errorbar(aes(x = Indicator, ymin = lowval, ymax = highval, col = Runtype),
                  position = position_dodge(width = .5), size = 1, width = 0.25) +
...
if (F){
  pdf("BCG_rr_all.pdf", width = 12, height = 7.5)
  ggplot(epi_BCG_2050[Indicator %in% rate_reductions]) +
    ...
    xlab(label = "Scenario") +
    scale_y_continuous(limits = c(0, 30), labels = scales::number_format(big.mark = ","))
  dev.off()
  
  pdf("BCG_averted_all.pdf", width = 12, height = 7.5)
...
raw <- fread("./epi_output/grouped_output_endTB/raw_output_endTB.csv")
raw <- raw[AgeGrp == "[0,99]" & Runtype == "baseline_endTB" &
             Indicator == "inc_rate"][, .(Year, AgeGrp, Runtype, Indicator,
                                          medval, lowval, highval)] 
...
if (T){ 
  ...
  epi_M72_endTB$Runtype <- ordered(epi_M72_endTB$Runtype, 
                                   ...
                                   labels = c("Basecase - Status Quo baseline",
                                              "Basecase - Strengthened Current Interventions baseline"))
...
if (F){
 ...
 # epi_M72_2050$facet = factor(epi_M72_2050$Indicator, levels = c("Cumulative Cases Averted", "Cumulative Treatments Averted",
  #                                           "Cumulative Deaths Averted","Incidence Rate Reduction", "Mortality Rate Reduction"))  
  ...
  M72plots <- ggplot(epi_M72_2050[Indicator %in% rate_reductions | Indicator %in% averted_numbers]) +
    ...
    geom_blank(data=data.frame(Indicator = "Cumulative Cases Averted",
                               Runtype = "Basecase",
                               medval = mr), aes(x = Runtype, y = medval)) +
  ...
  pdf(paste0("C:/Users/ChristinahMukandavir/Documents/manuscript/results/relative_output_plots2.pdf"), height = 11.7, width = 8.3)
  print(M72plots)
  print(BCGplots)
  dev.off()
