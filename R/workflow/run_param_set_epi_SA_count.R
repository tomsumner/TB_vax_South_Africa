run_param_set_epi <- function(model, cc, params, params_uid, vx_chars, vx_inc = NA) {
  ...
  output = model$run(model, new.parameter.values = params, baseline = scen_baseline, output.flows = F, write.to.file = F)
  ...
  #### Mortality
  mort <- counts[(TB == "TBdead" | TB == "TBHIVdead"),]
  ...
  #### HIVdead
  hivmort <- counts[HIV == "HIVdead" & (TB != "TBdead" | TB != "TBHIVdead"),]
  hivmort <- hivmort[, .(N_mort = sum(value)), by = .(Country = country, Year = year, AgeGrp)]
  hivmort <- hivmort[, Year := floor(Year)]
  ...
  n_epi <- n_epi[hivmort, on = .(Country = Country, Year = Year, AgeGrp = AgeGrp)]
