run_param_set_ltbi <- function(model, cc, params, params_uid, vx_chars, vx_inc = NA) {
  ...
  latentprevalence <- stocks[(TB == "L0" | TB == "Lf" | TB == "Ls")]
 ...
 # latentprevalenceUc <- stocks[(TB == "Lf" | TB == "Ls"  | TB == "Uc")]
  ...
  latentprevalenceR <- stocks[(TB == "Lf" | TB == "Ls"  | TB == "L0" | TB == "R")]
  latentprevalenceR <- latentprevalenceR[, .(latentprevalenceR = sum(value)), by = .(country, year, Age)]
  
  tbdeadbox <- c("TBdead", "TBHIVdead", "Rdead", "RHIVdead")
  alive_pop <- stocks[!(TB %in% tbdeadbox)]
