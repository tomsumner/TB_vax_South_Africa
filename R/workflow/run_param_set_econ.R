run_param_set_econ <- function(model, cc, params, params_uid, vx_chars, rr_pct) {
  ...
  if (T){
    ...
    inc      <- flows_ipj[dim == "TB" & TB == "Ds" & (HIV != "HIVdead") & flow == "in" 
                          & (subject != "TBp" & subject != "TBtr_init" & 
                               subject != "TBtr_nodeadreported" & subject != "TBtr_nodeadnotreported" &
                               subject != "TBtr_died"),]
    ...
    trt_succ <- flows_ipj[dim == "TB" & HIV != "HIVdead" &
                            ((TB == "R" & (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported"))
                             | (TB == "Dc" & (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported"))
                             | (TB == "TBdead" & subject == "TBtr_died")),]
    
    
    # 1. TREATMENT COMPLETION: nodeadR = flows into R from T
    SES_High_nodeadR  <- trt_succ[SES == "high" & dim == "TB" & (HIV != "HIVdead") & TB == "R"  &
                                    (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported")  & flow == "in",
                                  .(SES_High_complete = sum(value)), by = .(Country = country, Year = year, AgeGrp)]
    
    SES_Low_nodeadR  <- trt_succ[SES == "low" & dim == "TB" & (HIV != "HIVdead") & TB == "R"  &
                                   (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported")  & flow == "in",
    ...
    SES_High_nodeadDc <- trt_succ[SES == "high" & dim == "TB" & (HIV != "HIVdead") & TB == "Dc" &
                                    (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported")  & flow == "in",
                                  .(SES_High_nocomplete = sum(value)), by = .(Country = country, Year = year, AgeGrp)]
    
    SES_Low_nodeadDc <- trt_succ[SES == "low" & dim == "TB" & (HIV != "HIVdead") & TB == "Dc" &
                                   (subject == "TBtr_nodeadreported" | subject == "TBtr_nodeadnotreported")  & flow == "in",
    ...
    SES_High_deadT    <- trt_succ[SES == "high" & dim == "TB" & (HIV != "HIVdead") & TB == "TBdead"  &
                                    subject =="TBtr_died" & flow == "in",
                                  .(SES_High_dead = sum(value)), by = .(Country = country, Year = year, AgeGrp)]
    
    SES_Low_deadT    <- trt_succ[SES == "low" & dim == "TB" & (HIV != "HIVdead") & TB == "TBdead"  &
                                   subject =="TBtr_died" & flow == "in",
    ...
    # Number vaccinated = sum of subject == "VXaXi" & flow == "out"
    
    tb_vxa <- flows_ipj[dim == "VXa" & subject == "VXaXi" & flow == "out" &
                          TB != "Rdead" & TB!= "TBdead" & HIV != "HIVdead"]
    
    # Multiply by -1 because the flows "out" are negative
    tb_vxa <- tb_vxa[, value := (value*-1)]
  ...
  if(T){
    ...
    #stocks_ipj <- stocks_ipj[, .(Raw_Value = sum(value)),
    #                         by = .(Country = country, Year = year, AgeGrp, TB, HIV)]
