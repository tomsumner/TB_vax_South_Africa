...
###########################################################################################
.selec_simul<-function(summary_stat_target,param,simul,sd_simul,tol,accept=NULL,acceptance=FALSE,rejection=FALSE){
  dist=.compute_dist(summary_stat_target,simul[,1:length(summary_stat_target)],sd_simul)
  if(acceptance & !is.null(accept))
    {
      dist[accept==1] <- 0
    }
  if(rejection & !is.null(accept))
    {
      dist[accept==0] <- tol
    }
  dd=dim(param)[1]
  ...
  if (!is.null(dd)){
   if (ll > 1) {
    dist2 <- as.matrix(dist[dist < tol])
   } else {
     if (ll == 1){
    	dist2 <- as.matrix(dist[dist < tol])
	dim(dist2) <- c(dim(dist2)[2], dim(dist2)[1])
     } else {
  	dist2 <- NULL
     }
   }
...
.ABC_rejection_internal<-function(model, prior, nb_simul, use_seed, seed_count, verbose=FALSE, progressbarwidth=0, evalAccept=FALSE){
  options(scipen=50)
  ...
  if (evalAccept) {
    accept <- tab_simul_summarystat[,1]
    tab_simul_summarystat <- tab_simul_summarystat[,-1]
    if (is.null(nrow(tab_simul_summarystat)))
      {
        tab_simul_summarystat <- t(tab_simul_summarystat)
      }
  }
  
  list(param=as.matrix(tab_param), summarystat=as.matrix(tab_simul_summarystat), start=start, accept=accept)
}
...
#############################################################################################
.ABC_launcher_not_uniform_uni<-function(model,prior,param_previous_step,tab_unfixed_param,tab_weight,nb_simul,use_seed,seed_count,inside_prior,progressbarwidth=0,evalAccept=F){
  tab_simul_summarystat=NULL
  tab_param=NULL
  l=dim(param_previous_step)[2]
  lp=1
  ...
  if (evalAccept) {
    if (nrow(tab_simul_summarystat) == 1)
      {
        tab_simul_summarystat <- t(as.matrix(c(tab_simul_summarystat[-1], tab_simul_summarystat[1])))
      }
    else
      {
        tab_simul_summarystat <- cbind(tab_simul_summarystat[,-1], tab_simul_summarystat[,1])
      }
  }
...
#####################################################
.ABC_PMC<-function(model,prior,nb_simul,summary_stat_target,use_seed,verbose,seed_count=0,inside_prior=TRUE,tolerance_tab=-1,progress_bar=FALSE,acceptance=FALSE,rejection=FALSE){
  ...
  if (verbose){
    cat("    ------ Beaumont det al. (2009)'s algorithm ------\n")
  }
  progressbarwidth = 0
  if (progress_bar) {
    progressbarwidth = 50
  ...
  while (nb_simul_step>0){
    if (progress_bar) {
      cat("Step 1 of ", T, ". Rejection sampling, ",  nb_simul_step,
          " simulations\n", sep="")
      duration <- difftime(Sys.time(), start, units="secs")
      cat(paste("Time elapsed in this step:",
                format(.POSIXct(duration, tz="GMT"), "%H:%M:%S"),
                "Estimated time remaining:",
                format(.POSIXct(duration*nb_simul_step/(nb_simul-nb_simul_step),
                                tz="GMT"),
                       "%H:%M:%S"),
                "\n"))
    }
    if (nb_simul_step>1){
      # classic ABC step
      tab_ini=.ABC_rejection_internal(model,prior,nb_simul_step,use_seed,seed_count,progressbarwidth=progressbarwidth,evalAccept=(acceptance||rejection))
      ...
      simul_below_tol=rbind(simul_below_tol,.selec_simul(summary_stat_target,tab_ini$param,tab_ini$summarystat,sd_simul,tolerance_tab[1],tab_ini$accept,acceptance,rejection))
      ...
      tab_ini=.ABC_rejection_internal(model,prior,nb_simul_step,use_seed,seed_count,progressbarwidth=progressbarwidth,evalAccept=(acceptance || rejection))
      seed_count=seed_count+nb_simul_step
      dist=.compute_dist_single(summary_stat_target,tab_ini$summarystat[,1:nstat],sd_simul)
      if (((dist<tolerance_tab[1]) && (!rejection || tab_ini$accept==1)) || (acceptance && tab_ini$accept==1)){
        simul_below_tol=rbind(simul_below_tol,cbind(tab_ini$param, tab_ini$summarystat, dist))
        nb_simul_step=0
      }
    }
  ...
  for (it in 2:T){
    ...
    while (nb_simul_step>0){
      if (progress_bar) {
        cat("Step ", it, " of ", T,
            ". Sampling parameters around previous particles, ",
            nb_simul_step, " simulations.\n", sep = "")
        duration <- difftime(Sys.time(), start, units = "secs")
        cat(paste("Time elapsed in this step:",
                  format(.POSIXct(duration, tz = "GMT"), "%H:%M:%S"),
                  "Estimated time remaining:",
                  format(.POSIXct(duration * nb_simul_step /
	...
	tab_ini=.ABC_launcher_not_uniform_uni(model,prior,as.matrix(simul_below_tol[,1:nparam]),tab_unfixed_param,tab_weight,nb_simul_step,use_seed,seed_count,inside_prior,progressbarwidth=progressbarwidth,evalAccept=(acceptance || rejection))
        seed_count=seed_count+nb_simul_step
        simul_below_tol2=rbind(simul_below_tol2,.selec_simul(summary_stat_target,tab_ini[,1:nparam],tab_ini[,(nparam+1):(ncol(tab_ini)-1)],sd_simul,tolerance_tab[it],tab_ini[,ncol(tab_ini)],acceptance,rejection))
        if (length(simul_below_tol2)>0){
          nb_simul_step=nb_simul-dim(simul_below_tol2)[1]
        }
      }
      else{
        tab_ini=.ABC_launcher_not_uniform_uni(model,prior,as.matrix(simul_below_tol[,1:nparam]),tab_unfixed_param,tab_weight,nb_simul_step,use_seed,seed_count,inside_prior,progressbarwidth,acceptance || rejection)
        seed_count=seed_count+nb_simul_step
        dist=.compute_dist_single(summary_stat_target,tab_ini[(nparam+1):(nparam+nstat)],sd_simul)
        if (((dist<tolerance_tab[it]) && (!rejection || tab_ini[length(tab_ini)]==1)) || (acceptance && tab_ini[length(tab_ini)]==1)){
          simul_below_tol2=rbind(simul_below_tol2,cbind(t(tab_ini[1:(length(tab_ini)-1)]), dist))
          nb_simul_step=0
        }
      }
    ...
    simul_below_tol=matrix(0,nb_simul,ncol(simul_below_tol2))
    for (i1 in 1:nb_simul){
      for (i2 in 1:ncol(simul_below_tol2)){
  ...
  final_res=NULL
  if (verbose==TRUE){
  	final_res=list(param=as.matrix(simul_below_tol[,1:nparam]),stats=as.matrix(simul_below_tol[,(nparam+1):(ncol(simul_below_tol)-1)]),weights=tab_weight/sum(tab_weight),stats_normalization=as.numeric(sd_simul),epsilon=max(.compute_dist(summary_stat_target,as.matrix(simul_below_tol[,(nparam+1):(nparam+nstat)]),sd_simul)),nsim=(seed_count-seed_count_ini),computime=as.numeric(difftime(Sys.time(), start, units="secs")),intermediary=intermediary_steps)
  }
  else{
  	final_res=list(param=as.matrix(simul_below_tol[,1:nparam]),stats=as.matrix(simul_below_tol[,(nparam+1):(length(simul_below_tol)-1)]),weights=tab_weight/sum(tab_weight),stats_normalization=as.numeric(sd_simul),epsilon=max(.compute_dist(summary_stat_target,as.matrix(simul_below_tol[,(nparam+1):(nparam+nstat)]),sd_simul)),nsim=(seed_count-seed_count_ini),computime=as.numeric(difftime(Sys.time(), start, units="secs")))
  }
  final_res
}
...
#############################################
.ABC_MCMC<-function(model,prior,n_obs,n_between_sampling,summary_stat_target,use_seed,verbose,dist_max=0,tab_normalization=summary_stat_target,proposal_range=vector(mode = "numeric", length = length(prior)),seed_count=0,progress_bar=FALSE,acceptance=FALSE,rejection=FALSE,init_param=NULL){
  ...
  if (dist_max==0){
    ...
    if (acceptance || rejection) {
      simul_summary_stat <- simul_summary_stat[-1]
    }
  ...
  if (is.null(init_param)) {
      while (!test){
          ...
          accept <- NULL
          if (acceptance || rejection) {
              accept <- simul_summary_stat[1]
              simul_summary_stat <- simul_summary_stat[-1]
          }
          dist_simul=.compute_dist_single(summary_stat_target,as.numeric(simul_summary_stat),tab_normalization)
          if (((dist_simul<dist_max) && (!rejection || accept == 1)) || (acceptance && accept == 1)){
              test=TRUE
          }
          seed_count=seed_count+1
      }
  ...
  for (is in 2:n_obs){
    for (i in 1:n_between_sampling){
      ...
      dist_simul=.compute_dist_single(summary_stat_target,as.numeric(simul_summary_stat),tab_normalization)
      print(c(acceptance, accept,simul_summary_stat))
      if (((dist_simul<dist_max) && (!rejection || accept == 1)) || (acceptance && accept == 1)){
        print("ACCEPT")
        param_ini=param
        tab_simul_ini=as.numeric(simul_summary_stat)
        ...
        print("REJECT")
          tab_failed_param <- rbind(tab_failed_param, param)
      }
      seed_count=seed_count+1
    }
  ...
  tab_failed_param_matrix=NULL
  if (!is.null(tab_failed_param)) {
      tab_failed_param2=matrix(0,dim(tab_failed_param)[1],dim(tab_failed_param)[2])
      for (i in 1:dim(tab_failed_param)[1]){
          for (j in 1:dim(tab_failed_param)[2]){
              tab_failed_param2[i,j]=tab_failed_param[i,j]
          }
      }
      tab_failed_param_matrix=as.matrix(tab_failed_param2)
  }
  ...
  list(param=as.matrix(tab_param2),rejected_param=tab_failed_param_matrix,stats=as.matrix(tab_simul_summary_stat2),dist=tab_dist2,stats_normalization=as.numeric(tab_normalization),epsilon=max(tab_dist),nsim=(seed_count-seed_count_ini),n_between_sampling=n_between_sampling,computime=as.numeric(difftime(Sys.time(), start, units="secs")))
}
...
## function to perform ABC simulations from a non-uniform prior and with unidimensional jumps
#############################################################################################
.ABC_launcher_not_uniform_uni_cluster<-function(model,prior,param_previous_step,tab_unfixed_param,tab_weight,nb_simul,seed_count,inside_prior,n_cluster,evalAccept=F){
  tab_simul_summarystat=NULL
  tab_param=NULL
  cl <- makeCluster(getOption("cl.cores", n_cluster))
  list_param=list(NULL)
...
## PMC ABC algorithm: Beaumont et al. Biometrika 2009
#####################################################
.ABC_PMC_cluster<-function(model,prior,nb_simul,summary_stat_target,n_cluster,verbose,seed_count=0,inside_prior=TRUE,tolerance_tab=-1,progress_bar=FALSE,rejection=FALSE,acceptance=FALSE){
  ...
  if (progress_bar){
	print("    ------ Beaumont cet al. (2009)'s algorithm ------")
  }
  start = Sys.time()
  ...
  while (nb_simul_step>0){
    if (nb_simul_step>1){
      # classic ABC step
      tab_ini=.ABC_rejection_internal_cluster(model,prior,nb_simul_step,seed_count,n_cluster,rejection,acceptance)
      if (nb_simul_step==nb_simul){
        sd_simul=sapply(as.data.frame(tab_ini[,(nparam+1):(nparam+nstat)]),sd) # determination of the normalization constants in each dimension associated to each summary statistic, this normalization will not change during all the algorithm
      }
      seed_count=seed_count+nb_simul_step
