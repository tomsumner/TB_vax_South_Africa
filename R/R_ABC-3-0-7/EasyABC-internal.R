  ...
  function(sampleName,
    ...
    list(
      ...
      density = function(value) {
        do.call(densityName, as.list(as.numeric(c(
          value, densityArgs
        ))))
      },
      isUniform = isUniform ||
...
.create_legacy_prior <- function(name, args) {
  ...
  switch(
    EXPR = name,
    unif = .create_dynamic_prior(
      "runif", c(1, args), "dunif",
      args, TRUE
    ),
    normal = .create_dynamic_prior(
      "rnorm", c(1, args), "dnorm",
      args, TRUE
    ),
    lognormal = .create_dynamic_prior(
      "rlnorm", c(1, args), "dlnorm",
      args, TRUE
    ),
    exponential = .create_dynamic_prior(
      "rexp", c(1, args), "dexp",
      args, TRUE
    )
  )
  ...
  for (i in 1:l) {
    if (is.list(prior[[i]][1])) {
      if (length(prior[[i]]) != 2) {
        stop(
          paste(
            "Incorrect prior specification for '",
            prior[[i]],
      ...
      new_prior[[i]] <- .create_dynamic_prior(
        prior[[i]][[1]][1], prior[[i]][[1]][-1],
      ...
      if (any(prior[[i]][1] == c("unif", "normal", "lognormal", "exponential"))) {
        if (prior[[i]][1] == "exponential") {
          if (length(prior[[i]]) != 2) {
            stop(paste(
              "Incomplete prior information for parameter ",
              i,
              sep = ""
            ))
          }
        ...
        new_prior[[i]] <-
          .create_legacy_prior(prior[[i]][1], prior[[i]][-1])
      } else {
        stop(
          paste(
            "Only the methods 'unif', 'normal, 'lognormal' and 'exponential' are wrapped in EasyABC, unknown function '",
            prior[[i]][1],
            "'. Please refer to the documention for specifying your prior.",
            sep = ""
          )
        )
      }
...
.wrap_constants_in_model <- function(prior, model, use_seed) {
  ...
  for (i in 1:nb_parameters) {
    if ((prior[[i]][1] == "unif") &&
      ...
      constants_values <-
        append(constants_values, as.numeric(prior[[i]][2]))
    }
  }
...
.selec_simul <-
  function(summary_stat_target,
           ...
           dist_weights) {
    dist <-
      .compute_dist(summary_stat_target, simul, sd_simul, dist_weights = dist_weights)
...
## function to move a particle
.move_particleb_tbvax <- function(param_picked, varcov_matrix, prior, max_pick = 10000, noise_factor = 0.1) {

  # Propose particle using adapted jump kernel
  res <- 0.95 * MASS::mvrnorm(
    n = 1,
    mu = param_picked,
    Sigma = (2.56)^2 * varcov_matrix / length(param_picked), tol = 1e-4
  ) +
    0.05 * MASS::mvrnorm(
      n = 1,
      mu = param_picked,
      Sigma = (noise_factor)^2 * diag(length(param_picked)) / length(param_picked), tol = 1e-4
      
    )

  # Check if the proposed sample is in prior
  test <- .is_included(res, prior)

  # If in prior, return. If out of prior, return current point
  if (test) {
    return(res)
  } else {
    return(param_picked)
...
.is_in_parameter_constraints <- function(parameter, test) {
  is.null(test) ||
    eval(parse(text = gsub("[xX]([0-9]+)", "parameter[\\1]", test)))
}
  ...
  function(model,
    ...
    for (i in 1:nb_simul) {
      ...
      tab_simul_summarystat <-
        rbind(tab_simul_summarystat, as.numeric(simul_summarystat))
      if (use_seed) {
      ...
      if (!is.null(pb)) {
        ...
        text <- ""
        if (i == nb_simul) {
          text <- paste("Completed  in", format(
            .POSIXct(duration, tz = "GMT"),
            "%H:%M:%S"
          ), "                                              ")
        } else {
          text <- paste(
    ...
    options(scipen = 0)
    list(
      param = as.matrix(tab_param),
      summarystat = as.matrix(tab_simul_summarystat),
      start = start
    )
  }
...
.compute_weight <-
  function(param_simulated,
           param_previous_step,
           tab_weight,
           prior) {
    vmat <-
      as.matrix(2 * cov.wt(as.matrix(param_previous_step), as.vector(tab_weight))$cov)
    ...
    for (i in 1:n_particle) {
      for (k in 1:n_new_particle) {
        if (l > 1) {
          ...
          tab_weight_new[k] <-
            tab_weight_new[k] + tab_weight[i] * as.numeric(exp(-t(temp) %*%
              invmat %*% temp))
        } else {
          temp <- as.numeric(param_simulated[k]) - param_previous_step[i]
          tab_weight_new[k] <-
    ...
    tab_weight_prior <-
      .compute_weight_prior_tab(param_simulated, prior)
...
## function to move a particle with a unidimensional normal jump
.move_particleb_uni <-
  function(param_picked, sd_array, prior, max_pick = 100000) {
    test <- FALSE
    res <- param_picked
    counter <- 0
    while ((!test) && (counter < max_pick)) {
  ...
  function(param_simulated,
    ...
    if (!is.null(l)) {
      ...
      for (j in 1:l) {
        var_array[j] <-
          4 * diag(cov.wt(
            as.matrix(param_previous_step[, j]),
            as.vector(tab_weight)
          )$cov) # computation of a WEIGHTED variance
      ...
      multi <- (1 / sqrt(2 * pi))
      var_array <-
        4 * diag(cov.wt(as.matrix(param_previous_step), as.vector(tab_weight))$cov) # computation of a WEIGHTED variance
    ...
    for (i in 1:n_particle) {
      ...
      if (l > 1) {
        for (k in 1:l) {
          tab_temp <-
            tab_temp * exp(-(
              as.numeric(param_simulated[, k]) - as.numeric(param_previous_step[
                i,
                k
              ])
            ) * (
        ...
        tab_temp <-
          tab_temp * exp(-(
            as.numeric(param_simulated) - as.numeric(param_previous_step[i])
          ) *
            (
              as.numeric(param_simulated) - as.numeric(param_previous_step[i])
            ) / var_array)
...
.ABC_launcher_not_uniform_uni <-
  function(model,
    ...
    if (is.null(l)) {
      ...
      covmat <-
        2 * cov.wt(as.matrix(param_previous_step), as.vector(tab_weight))$cov # computation of a WEIGHTED variance
    } else {
      covmat <-
        2 * cov.wt(as.matrix(param_previous_step), as.vector(tab_weight))$cov # computation of a WEIGHTED variance
    }
    ...
    for (i in 1:nb_simul) {
      if (!inside_prior) {
        ...
        if (lp == 0) {
          param_picked <-
            .particle_pick(as.matrix(param_previous_step), tab_weight)
        } else {
          param_picked <-
            .particle_pick(as.matrix(param_previous_step), tab_weight)
        }
        ...
        if (lp == 0) {
          param_moved <-
            .move_particle_uni(as.numeric(param_picked), sd_array) # only variable parameters are moved
        } else {
          param_moved <-
            .move_particle_uni(as.numeric(param_picked), sd_array) # only variable parameters are moved
        }
      ...
      simul_summarystat <- model(param)
      tab_simul_summarystat <-
        rbind(tab_simul_summarystat, simul_summarystat)
      if (use_seed) {
        tab_param <- rbind(tab_param, param[2:(l + 1)])
      } else {
        tab_param <- rbind(tab_param, param)
  ...
  function(model,
    ...
    rejection <-
      .ABC_rejection_internal(
        ...
        seed_count,
        verbose,
        progressbarwidth = pgwidth
      )
    sd_simul <- sapply(as.data.frame(rejection$summarystat), sd)
    list(
      param = rejection$param,
      stats = as.matrix(rejection$summarystat),
      weights = array(
        1 / nb_simul,
        nb_simul
      ),
      stats_normalization = as.numeric(sd_simul),
      nsim = nb_simul,
      computime = as.numeric(difftime(Sys.time(),
        rejection$start,
        units = "secs"
      ))
    )
...
.ABC_PMC <-
  function(model,
    ...
    while (nb_simul_step > 0) {
      if (nb_simul_step > 1) {
        # classic ABC step
        tab_ini <- .ABC_rejection_internal(
          model,
          prior,
          prior_test,
          nb_simul_step,
        ...
        if (nb_simul_step == nb_simul) {
          sd_simul <-
            sapply(as.data.frame(tab_ini$summarystat), sd) # determination of the normalization constants in each dimension associated to each summary statistic, this normalization will not change during all the algorithm
        }
        ...
        seed_count <- seed_count + nb_simul_step
        if (.compute_dist(summary_stat_target,
          tab_ini$summarystat,
          sd_simul,
          dist_weights = dist_weights
        ) <
          ...
          simul_below_tol <-
            rbind(
              simul_below_tol,
              cbind(tab_ini$param, tab_ini$summarystat)
            )
          nb_simul_step <- 0
        }
      }
    ...
    if (verbose == TRUE) {
      write.table(
        as.matrix(cbind(tab_weight, simul_below_tol)),
        file = "output_step1",
        row.names = F,
        col.names = F,
        quote = F
      ...
      write.table(
        as.numeric(seed_count - seed_count_ini),
        file = "n_simul_tot_step1",
        row.names = F,
        col.names = F,
        quote = F
      )
      intermediary_steps[[1]] <- list(
        n_simul_tot = as.numeric(seed_count - seed_count_ini),
    ...
    for (it in 2:T) {
      ...
      tab_weight2 <- .compute_weight_uni(
        as.matrix(as.matrix(simul_below_tol2[
          ,
          1:nparam
        ])),
      ...
      if (verbose == TRUE) {
        ...
        intermediary_steps[[it]] <-
          list(
            n_simul_tot = as.numeric(seed_count -
              seed_count_ini),
            posterior = as.matrix(cbind(tab_weight, simul_below_tol))
          )
      }
    ...
    if (verbose == TRUE) {
      final_res <- list(
        param = as.matrix(simul_below_tol[, 1:nparam]),
        stats = as.matrix(simul_below_tol[
          ,
          (nparam + 1):(nparam + nstat)
        ]),
        weights = tab_weight / sum(tab_weight),
        stats_normalization = as.numeric(sd_simul),
        epsilon = max(
          .compute_dist(
            summary_stat_target,
            as.matrix(simul_below_tol[, (nparam + 1):(nparam + nstat)]),
            sd_simul,
            dist_weights = dist_weights
          )
        ),
        nsim = (seed_count - seed_count_ini),
        computime = as.numeric(difftime(Sys.time(),
          start,
          units = "secs"
        )),
        intermediary = intermediary_steps
      )
    } else {
      final_res <- list(
        param = as.matrix(simul_below_tol[, 1:nparam]),
...
.ABC_Drovandi <-
  function(model,
           ...
           alpha = 0.5,
           c = 0.01,
           first_tolerance_level_auto = TRUE,
           dist_weights = NULL,
           seed_count = 0,
           progress_bar = FALSE,
           max_pick = 10000) {
    ...
    if (first_tolerance_level_auto) {
      ...
      simul_below_tol <-
        as.matrix(simul_below_tol[1:nb_simul, ]) # to be sure that there are not two or more simulations at a distance equal to the tolerance determined by the quantile
    } else {
      nb_simul_step <- nb_simul
      while (nb_simul_step > 0) {
        if (nb_simul_step > 1) {
          ...
          simul_below_tol <-
            ...
            ) # we authorize the simulation to be equal to the tolerance level, for consistency with the quantile definition of the tolerance
          if (length(simul_below_tol) > 0) {
            nb_simul_step <- nb_simul - dim(simul_below_tol)[1]
          }
        } else {
    ...
    if (first_tolerance_level_auto) {
      tol_next <-
        max(
          .compute_dist(
            summary_stat_target,
            as.matrix(as.matrix(simul_below_tol)[
              ,
              (nparam + 1):(nparam + nstat)
            ]),
            sd_simul,
    ...
    while (tol_next > tol_end) {
      ...
      simul_below_tol2 <-
        .selec_simulb(
          ...
          as.matrix(as.matrix(simul_below_tol)[, (nparam + 1):(nparam +
            nstat)]),
          sd_simul,
          tol_next,
          dist_weights = dist_weights
      ...
      simul_below_tol <-
        matrix(0, (nb_simul - n_alpha), (nparam + nstat))
      ...
      for (i in 1:nb_simul_step) {
        # pick a particle
        simul_picked <-
          .particle_pick(simul_below_tol, tab_weight[1:(nb_simul -
            n_alpha)])
        for (j in 1:R) {
          # move it
          param_moved <-
            .move_particleb(simul_picked[1:nparam], 2 * var(as.matrix(as.matrix(
              simul_below_tol[
                ,
                1:nparam
              ]
        ...
        if (progress_bar) {
          ...
          if (i == nb_simul_step) {
            ...
            text <- paste(
              ...
              "Estimated time remaining for step ",
              it,
              ":",
              format(
                .POSIXct(
                  duration / i * (nb_simul_step - i),
                  tz = "GMT"
                ),
                "%H:%M:%S"
              )
      ...
      for (i1 in (nb_simul - n_alpha + 1):nb_simul) {
        for (i2 in 1:(nparam + nstat)) {
          simul_below_tol3[i1, i2] <-
            as.numeric(simul_below_tol2[(i1 - nb_simul +
              n_alpha), i2])
        }
      }
      simul_below_tol <- simul_below_tol3
      p_acc <-
        max(1, i_acc) / (nb_simul_step * R) # to have a strictly positive p_acc
      Rp <- R
      if (p_acc < 1) {
      ...
      if (verbose == TRUE) {
        ...
        intermediary_steps[[it]] <- list(
          n_simul_tot = as.numeric(seed_count -
            seed_count_ini),
          R_step = as.numeric(Rp),
    ...
    for (i in 1:nb_simul) {
      ...
      seed_count <- seed_count + R
      simul_below_tol2 <-
        rbind(simul_below_tol2, as.numeric(simul_picked))
    }
    final_res <- NULL
    if (verbose == TRUE) {
      final_res <- list(
        param = as.matrix(simul_below_tol2[, 1:nparam]),
        stats = as.matrix(simul_below_tol2[
          ,
          (nparam + 1):(nparam + nstat)
        ]),
        weights = tab_weight / sum(tab_weight),
...
.compute_dist_M <-
  function(M,
    ...
    if (l > 1) {
      for (i in 1:l) {
        vartab[i] <-
          min(1, 1 / (sd_simul[i] * sd_simul[i])) ## differences between simul and data are normalized in each dimension by the empirical variances in each dimension
        dist <-
          dist + vartab[i] * (simul[, i] - summary_stat_target[i]) * (simul[
            ,
            i
          ] - summary_stat_target[i]) ## an euclidean distance is used
      }
    } else {
      vartab <-
        min(1, 1 / (sd_simul * sd_simul)) ## differences between simul and data are normalized in each dimension by the empirical variances in each dimension
      dist <-
        dist + vartab * (simul[, 1] - summary_stat_target) * (simul[, 1] - summary_stat_target) ## an euclidean distance is used
    }
...
.compute_weight_delmoral <- function(particle_dist_mat, tolerance) {
  ...
  for (i in 1:n_particle) {
    new_weight[i] <-
      length(particle_dist_mat[i, ][particle_dist_mat[i, ] < tolerance])
  }
...
.particle_pick_delmoral <-
  function(simul_below_tol, tab_weight, M) {
    u <- runif(1)
...
.ABC_Delmoral <-
  function(model,
    ...
    simul_below_tol <- .ABC_rejection_M(
      model, prior, prior_test, nb_simul, M, use_seed,
      seed_count
    )
    ...
    ESS <- nb_simul
    uu <-
      (1:nb_simul) * M # to compute sd_simul with only one simulation per parameter set
    sd_simul <- sapply(
      as.data.frame(simul_below_tol[uu, (nparam + 1):(nparam + nstat)]),
      sd
    ...
    if (M > 1) {
      ...
      particle_dist_mat <-
        .compute_dist(summary_stat_target,
          as.matrix(as.matrix(simul_below_tol)[
            ,
            (nparam + 1):(nparam + nstat)
          ]),
    ...
    if (verbose == TRUE) {
      write.table(
        as.matrix(cbind(tab_weight2, simul_below_tol)),
        file = "output_step1",
        row.names = F,
        col.names = F,
        quote = F
    ...
    while (new_tolerance > tolerance_target) {
      ...
      tab_weight <-
        .compute_weight_delmoral(particle_dist_mat, new_tolerance)
      ...
      if (ESS < nb_threshold) {
        ...
        for (i in 1:nb_simul) {
          particles[((1:M) + (i - 1) * M), ] <-
            as.matrix(.particle_pick_delmoral(
              simul_below_tol,
              tab_weight, M
            ))
        }
      ...
      # MCMC move
      covmat <-
        2 * cov.wt(as.matrix(as.matrix(particles[uu, ])[tab_weight > 0, ]), as.vector(tab_weight[tab_weight > 0]))$cov
      ...
      for (i in 1:nb_simul) {
        if (tab_weight[i] > 0) {
          tab_new_simul <- NULL
          # move it
          param_moved <- .move_particleb_uni(
            as.numeric(particles[(i * M), ]),
          ...
          if (M > 1) {
            new_dist <-
              .compute_dist_M(M,
                summary_stat_target,
                as.matrix(as.matrix(tab_new_simul2)[
                  ,
                  (nparam + 1):(nparam + nstat)
                ]),
                sd_simul,
          ...
          if (uuu <= MH) {
            for (i1 in 1:M) {
              for (i2 in 1:(nparam + nstat)) {
                simul_below_tol[((i1) + (i - 1) * M), i2] <-
                  as.numeric(tab_new_simul2[
                    i1,
                    i2
                  ])
              }
            ...
            for (i1 in 1:M) {
              for (i2 in 1:(nparam + nstat)) {
                simul_below_tol[((i1) + (i - 1) * M), i2] <-
                  as.numeric(simul_below_tol2[((i1) +
                    (i - 1) * M), i2])
              }
            }
          }
      ...
      if (verbose == TRUE) {
        write.table(
          as.numeric(new_tolerance),
          file = paste("tolerance_step",
            kstep,
            sep = ""
          ),
          row.names = F,
        ...
        intermediary_steps[[kstep]] <-
          list(
            n_simul_tot = as.numeric(seed_count -
              seed_count_ini),
            tol_step = as.numeric(new_tolerance),
            posterior = as.matrix(cbind(
              tab_weight2,
              simul_below_tol
            ))
          )
    ...
    if (verbose == TRUE) {
      final_res <- list(
        param = as.matrix(as.matrix(simul_below_tol)[, 1:nparam]),
        stats = as.matrix(as.matrix(simul_below_tol)[, (nparam + 1):(nparam +
          nstat)]),
        weights = tab_weight2 / sum(tab_weight2),
        stats_normalization = as.numeric(sd_simul),
        epsilon = max(
          .compute_dist(
            summary_stat_target,
        ...
        nsim = (seed_count -
          seed_count_ini),
        computime = as.numeric(difftime(Sys.time(), start,
          units = "secs"
        )),
        intermediary = intermediary_steps
      )
  ...
  function(model,
    ...
    for (i in 1:nb_simul) {
      ...
      while (!test_passed) {
        ...
        for (j in 1:l) {
          param[j] <-
            as.numeric(prior[[j]]$sampleArgs[2]) + (as.numeric(prior[[j]]$sampleArgs[3]) -
...
.compute_weightb_uni <-
  function(param_simulated,
    ...
    for (j in 1:l) {
      var_array[j] <-
        diag(4 * cov.wt(as.matrix(param_previous_step[, j]), as.vector(tab_weight))$cov)
      multi <- multi * (1 / sqrt(var_array[j] / 2))
    }
    var_array <- as.numeric(var_array)
    n_particle <- dim(param_previous_step)[1]
  ...
  function(model,
    ...
    for (i in 1:nb_simul) {
      ...
      if (!inside_prior) {
        ...
        # pick a particle
        param_picked <-
          .particle_pick(param_previous_step, tab_weight)
        # move it
        param_moved <-
          .move_particle(
            as.numeric(param_picked),
            2 * cov.wt(
              as.matrix(as.matrix(
                param_previous_step
              )),
...
.ABC_Lenormand <-
  function(model,
    ...
    if (!store) {
      ...
      if (!.all_unif(prior)) {
        stop(
          "Prior distributions must be uniform to use the Lenormand et al. (2012)'s algorithm."
        )
      }
      n_alpha <- ceiling(nb_simul * alpha)
      ## step 1 ABC rejection step with LHS
      tab_ini <- .ABC_rejection_lhs(
        model, prior, prior_test, nb_simul, use_seed,
        seed_count
      )
      # initially, weights are equal
      ...
      if (verbose == TRUE) {
        write.table(
          as.matrix(cbind(tab_weight, tab_ini)),
          file = "model_step1",
          row.names = F,
          col.names = F,
          quote = F
      ...
      # algorithm
      sd_simul <-
        sapply(as.data.frame(tab_ini[, (nparam + 1):(nparam + nstat)]),
          sd,
          na.rm = TRUE
        )
      ...
      simul_below_tol <-
        rbind(
          ...
          .selec_simul_alpha(
            summary_stat_target,
            as.matrix(as.matrix(tab_ini)[, 1:nparam]),
            as.matrix(as.matrix(tab_ini)[
              ,
              (nparam + 1):(nparam + nstat)
            ]),
            sd_simul,
      ...
      simul_below_tol <-
        simul_below_tol[1:n_alpha, ] # to be sure that there are not two or more simulations at a distance equal to the tolerance determined by the quantile
      tab_dist <-
        .compute_dist(summary_stat_target,
          as.matrix(as.matrix(simul_below_tol)[
            ,
      ...
      if (verbose == TRUE) {
        ...
        write.table(
          as.numeric(tol_next),
          file = "tolerance_step1",
          row.names = F,
          col.names = F,
          quote = F
        )
        intermediary_steps[[1]] <-
          list(
            n_simul_tot = as.numeric(seed_count -
              seed_count_ini),
            tol_step = as.numeric(tol_next),
      ...
      while (p_acc > p_acc_min) {
        it <- it + 1
        simul_below_tol2 <- NULL
        tab_inic <- .ABC_launcher_not_uniformc(
          model,
          prior,
          as.matrix(as.matrix(simul_below_tol)[
            ,
        ...
        if (!inside_prior) {
          ...
          tab_weight2 <- tab_inic[[2]] * (.compute_weightb(
            as.matrix(as.matrix(tab_ini[
              ,
              1:nparam
            ])),
        ...
        if (verbose == TRUE) {
          write.table(
            as.matrix(cbind(tab_weight2, tab_ini)),
            file = paste("model_step",
              it,
              sep = ""
            ),
            row.names = F,
        ...
        simul_below_tol2 <-
          rbind(as.matrix(simul_below_tol), as.matrix(tab_ini))
        ...
        p_acc <-
          length(tab_dist2[tab_dist2 <= tol_next]) / nb_simul_step
        ...
        for (i1 in 1:n_alpha) {
          ...
          for (i2 in 1:(nparam + nstat)) {
            simul_below_tol[i1, i2] <- as.numeric(simul_below_tol2[
              odist[i1],
              i2
            ])
          }
      ...
      seed_count <- seed_count + nb_simul
      sd_simul <- sapply(
        as.data.frame(tab_ini[, (nparam + 1):(nparam + nstat)]),
        sd
      ) # determination of the normalization constants in each dimension associated to each summary statistic, this normalization will not change during all the algorithm
      write.table(
        as.matrix(cbind(
          array(1, nb_simul), as.matrix(tab_ini)
        )),
        file = "output_all",
        row.names = F,
        col.names = F,
      ...
      while (p_acc > p_acc_min) {
        ...
        simul_below_tol2 <-
          rbind(as.matrix(simul_below_tol), as.matrix(tab_ini))
        write.table(
          as.matrix(cbind(tab_weight2, as.matrix(tab_ini))),
          file = "output_all",
          row.names = F,
          col.names = F,
          quote = F,
...
.predict_locreg_deg2 <-
  function(x, design_pts, design_stats, span) {
    ...
    if (is.vector(centered_pts)) {
      ...
      reg_deg2 <- as.formula(paste(
        "design_stats ~ (", paste(xnam, collapse = "]+"),
        "])^2"
      ))
    }
...
.ABC_sequential_emulation <-
  function(model,
    ...
    ## step 1 ABC rejection step with LHS
    tab_ini <-
      .ABC_rejection_lhs(model, prior, prior_test, nb_simul, use_seed, seed_count)
    seed_count <- seed_count + nb_simul
    # determination of the normalization constants in each dimension associated to
    # each summary statistic, this normalization will not change during all the
    # algorithm
    sd_simul <-
      sapply(as.data.frame(tab_ini[, (nparam + 1):(nparam + nstat)]), sd)
    ...
    for (i_step_emulation in 1:n_step_emulation) {
      ...
      res_emulator <-
        .ABC_sequential_emulator(
          ...
          emulator_design_stats,
          span,
          tab_ini[tab_dist <= tol_max, ],
          tab_weight_end[tab_dist <= tol_max],
          nparam,
          nstat,
          sd_simul,
          prior,
      ...
      new_particles <- .ABC_sequential_emulation_follow(
        model,
        res_emulator$weights,
        res_emulator$simul,
        nparam,
        nstat,
        prior,
    ...
    final_res <- list(
      ...
      stats = as.matrix(as.matrix(tab_ini)[
        ,
        (nparam + 1):(nparam + nstat)
      ]),
      weights = tab_weight_end,
      stats_normalization = as.numeric(sd_simul),
...
.ABC_sequential_emulation_follow <-
  function(model,
    ...
    final_res <-
      list(simul = as.matrix(as.matrix(tab_ini)), weights = tab_weight2)
    final_res
  }
...
.ABC_sequential_emulator <-
  function(model,
    ...
    while (p_acc > p_acc_min) {
      ...
      if (use_seed) {
        model_emulator <- function(parameters) {
          .emulator_locreg_deg2(
            parameters[2:length(parameters)],
            emulator_design_pts,
            emulator_design_stats,
            emulator_span
          )
    ...
    final_res <-
      list(simul = as.matrix(as.matrix(simul_below_tol)), weights = tab_weight)
    final_res
  }
...
.ABC_sequential <-
  function(method,
    ...
    return(switch(
      ...
      Emulation = .ABC_sequential_emulation(
        model,
        prior,
        prior_test,
        nb_simul,
...
.move_particle_uni_uniform <-
  function(param_picked, sd_array, prior, max_pick = 10000) {
    test <- FALSE
    res <- param_picked
    counter <- 0
    while ((!test) && (counter < max_pick)) {
...
.ABC_MCMC <-
  function(model,
    ...
    if (sum(abs(tab_normalization - summary_stat_target)) == 0) {
      print(
        "Warning: summary statistics are normalized by default through a division by the target summary statistics - it may not be appropriate to your case."
      )
      print(
        "Consider providing normalization constants for each summary statistics in the option 'tab_normalization' or using the method 'Marjoram' which automatically determines these constants."
      )
    }
    ...
    if (sum(proposal_range) == 0) {
      ...
      print(
        "Warning: default values for proposal distributions are used - they may not be appropriate to your case."
      )
      print(
        "Consider providing proposal range constants for each parameter in the option 'proposal_range' or using the method 'Marjoram' which automatically determines these constants."
      )
    }
    seed_count_ini <- seed_count
    nparam <- length(prior)
    ...
    if (dist_max == 0) {
      ...
      dist_simul <- .compute_dist(summary_stat_target, as.numeric(simul_summary_stat), tab_normalization, dist_weights = dist_weights)
    ...
    if (is.null(init_param)) {
      while (!test) {
        ...
        simul_summary_stat <- model(param)
        dist_simul <- .compute_dist(summary_stat_target,
                                    as.numeric(simul_summary_stat),
                                    tab_normalization,
                                    dist_weights = dist_weights)
      ...
      simul_summary_stat <- model(param)
      dist_simul <- .compute_dist(summary_stat_target,
                                  as.numeric(simul_summary_stat), tab_normalization,
                                  dist_weights = dist_weights)
      if (dist_simul < dist_max) {
        test <- TRUE
      }
    ...
    tab_simul_summary_stat <-
      rbind(tab_simul_summary_stat, simul_summary_stat)
    tab_param <- rbind(tab_param, param)
    if (use_seed) {
    ...
    for (is in 2:n_obs) {
      ...
      for (i in 1:n_between_sampling) {
        ...
        if (adaptive$toggle) {
          if (is < adaptive$burn) {
            param <- .move_particle_uni_uniform(
              as.numeric(param_ini),
              proposal_range,
              prior,
              max_pick
            ...
            # If cov_int reached, or update toggle on, generate covariance matrix
            cat("Adaptive live status:", adaptive$update, "\n")
            if (is %% adaptive$cov_int == 0 || adaptive$update) {
              cat("adjust covm.\n")
              if (adaptive$sample_width == Inf) {
                covmatrix <<- cov(tab_param[2:(is - 1), ])
              } else {
                covmatrix <<- cov(tab_param[(is - (adaptive$sample_width - 2)):(is - 1), ])
              }
            }
            tryCatch(expr = {
              param <- .move_particleb_tbvax(
                as.numeric(param_ini),
                covmatrix,
                prior,
                noise_factor = adaptive$noise_factor
              )
              
              # Since the proposal did not fail, switch off update toggle.
              adaptive$update <- FALSE
              
            }, error = function(e) {
              
              cat("Gaussian jump kernel failed at iteration: ", is, "\n")
              cat("error:", e$message, "\n")
              cat("continuing with uniform jump", "\n")
              ...
              # Since the proposal failed, try to update covariance
              adaptive$update <<- TRUE
              # cat("retry covariance matrix update\n")
              # if (adaptive$sample_width == Inf) {
              #   covmatrix <<- cov(tab_param[2:(is - 1), ])
              # } else {
              #   covmatrix <<- cov(tab_param[(is - (adaptive$sample_width - 2)):(is - 1), ])
        ...
        } else if (isFALSE(adaptive$toggle)) {
          param <- .move_particle_uni_uniform(
            as.numeric(param_ini), proposal_range,
            prior, max_pick
          )
        }
      ...
      tab_simul_summary_stat <-
        rbind(tab_simul_summary_stat, tab_simul_ini)
    ...
    tab_simul_summary_stat2 <-
      matrix(
        0,
        dim(tab_simul_summary_stat)[1],
        dim(tab_simul_summary_stat)[2]
      )
    ...
    list(
      param = as.matrix(tab_param2),
      stats = as.matrix(tab_simul_summary_stat2),
      dist = tab_dist2,
      stats_normalization = as.numeric(tab_normalization),
      epsilon = max(tab_dist),
      nsim = (seed_count - seed_count_ini),
...
.ABC_MCMC2 <-
  function(model,
    ...
    if (progress_bar) {
      print(
        "    ------ Marjoram et al. (2003)'s algorithm with modifications drawn from Wegmann et al. (2009) related to automatization ------"
      )
    }
    seed_count_ini <- seed_count
    nparam <- length(prior)
    nstat <- length(summary_stat_target)
    tab_param <-
      matrix(NA, n_calibration, ifelse(use_seed, nparam + 1, nparam))
    ...
    for (i in 1:nparam) {
      proposal_range[i] <-
        sd(as.matrix(tab_param)[, i]) * proposal_phi / 2
    }
    n_ini <- sample(nmax, 1)
    tab_simul_ini <-
      as.numeric(tab_simul_summary_stat[(ord_sim[n_ini]), ])
    dist_ini <- simuldist[(ord_sim[n_ini])]
    param_ini <- as.matrix(tab_param)[n_ini, ]
    if (verbose == TRUE) {
      write.table((seed_count - seed_count_ini),
        file = "n_simul_tot_step1",
        row.names = F,
        col.names = F,
        quote = F
...
.ABC_MCMC3 <-
  function(model,
    ...
    for (i in 1:nstat) {
      ...
      summary_stat_targ[i] <-
        1 + (summary_stat_targ[i] - myMin[i]) / (myMax[i] -
          myMin[i])
    }
    ...
    for (i in 1:nstat) {
      ...
      mylm <-
        lm(as.formula(as.data.frame(dmat)), data = as.data.frame(dmat))
      # mylm<-lm(as.formula(as.data.frame(dmat))) mylm<-lm(stat[,i]~as.matrix(sparam))
      myboxcox <- boxcox(
        mylm,
        lambda = seq(-20, 100, 1 / 10),
        interp = T,
        eps = 1 / 50,
        plotit = FALSE
      )
    ...
    for (i in 1:nstat) {
      stat[, i] <-
        ((stat[, i]^lambda[i]) - 1) / (lambda[i] * (myGM[i]^(lambda[i] -
          1)))
      summary_stat_targ[i] <-
        ((summary_stat_targ[i]^lambda[i]) - 1) / (lambda[i] *
          (myGM[i]^(lambda[i] - 1)))
      myBCSDs <- c(myBCSDs, sd(stat[, i]))
      myBCMeans <- c(myBCMeans, mean(stat[, i]))
      stat[, i] <- (stat[, i] - myBCMeans[i]) / myBCSDs[i]
      summary_stat_targ[i] <-
        (summary_stat_targ[i] - myBCMeans[i]) / myBCSDs[i]
    }
    # perform pls
    myPlsr <- plsr(
      as.matrix(sparam) ~ as.matrix(stat),
      scale = F,
      ncomp = numcomp,
      validation = "LOO"
    )
    ...
    ## AM3 print('AM3 ')
    summary_stat_targ <-
      t(pls_transformation %*% as.vector(summary_stat_targ))
    stat_pls <- t(pls_transformation %*% t(stat))
    simuldist <-
      .compute_dist(summary_stat_targ, stat_pls, rep(1, numcomp), dist_weights = dist_weights)
    ## AM4 print('AM4 ')
    ord_sim <- order(simuldist, decreasing = F)
    nmax <- ceiling(tolerance_quantile * n_calibration)
    dist_max <- simuldist[(ord_sim[nmax])]
    ...
    for (is in 2:n_obs) {
      for (i in 1:n_between_sampling) {
        ...
        for (ii in 1:nstat) {
          simul_summary_stat[ii] <-
            1 + (simul_summary_stat[ii] - myMin[ii]) / (myMax[ii] -
              myMin[ii])
        }
        for (ii in 1:nstat) {
          ...
          simul_summary_stat[ii] <-
            (simul_summary_stat[ii] - myBCMeans[ii]) / myBCSDs[ii]
        }
        simul_summary_stat <- as.matrix(simul_summary_stat)
        dim(simul_summary_stat) <- c(nstat, 1)
        simul_summary_stat <-
          t(pls_transformation %*% simul_summary_stat)
        dist_simul <-
          .compute_dist(
            summary_stat_targ,
            as.numeric(simul_summary_stat),
    ...
    list(
      ...
      lambda = lambda,
      geometric_mean = myGM,
      boxcox_mean = myBCMeans,
      boxcox_sd = myBCSDs,
      pls_transform = pls_transformation,
      n_component = numcomp,
      computime = as.numeric(difftime(Sys.time(),
        start,
        units = "secs"
...
.ABC_rejection_cluster <-
  function(model,
    ...
    if (npar > 0) {
      for (irun in 1:npar) {
        ...
        for (i in 1:(100 * n_cluster)) {
          ...
          simultemp <-
            rbind(simultemp, as.numeric(list_simul_summarystat[[i]]))
        }
        if (verbose) {
    ...
    list(
      param = as.matrix(tab_param),
      stats = as.matrix(tab_simul_summarystat),
      weights = array(1 / nb_simul, nb_simul),
      stats_normalization = as.numeric(sd_simul),
      nsim = nb_simul,
      computime = as.numeric(difftime(Sys.time(), start, units = "secs"))
    )
    ...
    if (npar > 0) {
      for (irun in 1:npar) {
        for (i in 1:(100 * n_cluster)) {
          ...
          if (!inside_prior) {
            # pick a particle
            param_picked <- .particle_pick(
              as.matrix(as.matrix(param_previous_step)),
              tab_weight
            )
            # move it
            param_moved <-
...
.ABC_launcher_not_uniform_uni_cluster <-
  function(model,
    ...
    sd_array <- array(1, l_array)
    covmat <-
      2 * cov.wt(as.matrix(as.matrix(param_previous_step)), as.vector(tab_weight))$cov # computation of a WEIGHTED variance
    for (j in 1:l_array) {
      sd_array[j] <- sqrt(covmat[j, j])
    }
    if (npar > 0) {
...
.ABC_PMC_cluster <-
  function(model,
    ...
    while (nb_simul_step > 0) {
      if (nb_simul_step > 1) {
        # classic ABC step
        tab_ini <- .ABC_rejection_internal_cluster(
          model,
          prior,
          prior_test,
          nb_simul_step,
    ...
    for (it in 2:T) {
      ...
      while (nb_simul_step > 0) {
        if (nb_simul_step > 1) {
          # Sampling of parameters around the previous particles
          tab_ini <- .ABC_launcher_not_uniform_uni_cluster(
            model,
            prior,
            as.matrix(as.matrix(simul_below_tol)[
              ,
    ...
    if (verbose == TRUE) {
      final_res <- list(
        param = as.matrix(as.matrix(simul_below_tol)[, 1:nparam]),
        stats = as.matrix(as.matrix(as.matrix(
          simul_below_tol
        ))[, (nparam + 1):(nparam +
          nstat)]),
        weights = tab_weight / sum(tab_weight),
    ...
    if (npar > 0) {
      for (irun in 1:npar) {
        ...
        for (i in 1:(100 * n_cluster)) {
          # pick a particle
          simul_picked <-
            new_particles[((irun - 1) * n_cluster + i), ]
          # move it
          param_moved <-
            .move_particleb(simul_picked[1:nparam], 2 * var(as.matrix(as.matrix(
              as.matrix(new_particles)[
                ,
                1:nparam
              ]
            ))), prior, max_pick)
...
.ABC_Drovandi_cluster <-
  function(model,
    ...
    if (first_tolerance_level_auto) {
      ...
      simul_below_tol <-
        simul_below_tol[1:nb_simul, ] # to be sure that there are not two or more simulations at a distance equal to the tolerance determined by the quantile
    } else {
      nb_simul_step <- nb_simul
      while (nb_simul_step > 0) {
        if (nb_simul_step > 1) {
    ...
    while (tol_next > tol_end) {
      ...
      md <- .move_drovandi_ini_cluster(
        nb_simul_step,
        simul_below_tol,
        tab_weight[1:(nb_simul -
          n_alpha)],
        nparam,
        nstat,
        prior,
      ...
      for (i1 in (nb_simul - n_alpha + 1):nb_simul) {
        for (i2 in 1:(nparam + nstat)) {
          simul_below_tol3[i1, i2] <-
            as.numeric(new_particles[(i1 - nb_simul +
              n_alpha), i2])
        }
      }
      simul_below_tol <- simul_below_tol3
    ...
    simul_below_tol2 <- NULL
    for (j in 1:R) {
      simul_below_tol <- .move_drovandi_diversify_cluster(
        nb_simul,
        simul_below_tol,
        nparam,
        nstat,
...
.ABC_Delmoral_cluster <-
  function(model,
    ...
    nstat <- length(summary_stat_target)
    # step 1 classic ABC step
    simul_below_tol <- .ABC_rejection_M_cluster(
      model, prior, prior_test, nb_simul,
    ...
    while (new_tolerance > tolerance_target) {
        ...
        for (irun in 1:npar) {
          ...
          for (ii in 1:ic) {
            tab_simul_summarystat <-
              rbind(
                tab_simul_summarystat,
                as.numeric(list_simul_summarystat[[ii]])
              )
          }
        }
      }
      ...
      tab_new_simul2 <-
        matrix(0, dim(tab_new_simul)[1], dim(tab_new_simul)[2])
          ...
          if (M > 1) {
            new_dist <-
              .compute_dist_M(M,
                summary_stat_target,
                tab_new_simul2[(((iii -
                  1) * M + 1):(iii * M)), (nparam + 1):(nparam + nstat)],
                sd_simul,
                dist_weights = dist_weights
              )
            n_acc <- length(new_dist[new_dist < new_tolerance])
          } else {
            new_dist <- .compute_dist(
              summary_stat_target,
              rbind(tab_new_simul2[
                iii,
                (nparam + 1):(nparam + nstat)
              ], tab_new_simul2[iii, (nparam +
                1):(nparam + nstat)]),
              sd_simul,
              dist_weights = dist_weights
            )
    ...
    list(
      param = as.matrix(as.matrix(simul_below_tol)[, 1:nparam]),
      stats = as.matrix(as.matrix(simul_below_tol)[
        ,
        (nparam + 1):(nparam + nstat)
      ]),
      weights = tab_weight2 / sum(tab_weight2),
      ...
      for (irun in 1:npar) {
        for (i in 1:(100 * n_cluster)) {
          ...
          repeat {
            ...
            if ((!inside_prior) ||
              (.is_included(param_moved, prior)) || (counter >= max_pick)) {
              break
            }
          }
          if (counter == max_pick) {
...
.ABC_Lenormand_cluster <-
  function(model,
    ...
    n_alpha <- ceiling(nb_simul * alpha)
    ## step 1 ABC rejection step with LHS
    tab_ini <- .ABC_rejection_lhs_cluster(
      model, prior, prior_test, nb_simul, seed_count,
      n_cluster
    )
    # initially, weights are equal
    tab_weight <- array(1, n_alpha)
    ...
    while (p_acc > p_acc_min) {
      it <- it + 1
      simul_below_tol2 <- NULL
      tab_inic <- .ABC_launcher_not_uniformc_cluster(
        model,
        prior,
        as.matrix(as.matrix(simul_below_tol)[
          ,
      ...
      if (!inside_prior) {
        tab_weight2 <- .compute_weightb(
          as.matrix(as.matrix(as.matrix(tab_ini)[
            ,
            1:nparam
          ])),
          as.matrix(as.matrix(as.matrix(
      ...
      p_acc <-
        length(tab_dist2[!is.na(tab_dist2) &
          tab_dist2 <= tol_next]) / nb_simul_step
      tab_dist <- c(tab_dist, tab_dist2)
      tol_next <- sort(tab_dist)[n_alpha]
      simul_below_tol2 <-
        simul_below_tol2[!is.na(tab_dist) & tab_dist <= tol_next, ]
      tab_weight <-
        tab_weight[!is.na(tab_dist) & tab_dist <= tol_next]
      tab_weight <- tab_weight[1:n_alpha]
...
.ABC_sequential_cluster <-
  function(method,
    ...
    if (use_seed == FALSE) {
      stop(
        "For parallel implementations, you must specify the option 'use_seed=TRUE' and modify your model accordingly - see the package's vignette for more details."
      )
    }
    options(scipen = 50)
    return(switch(
      EXPR = method,
      Beaumont = .ABC_PMC_cluster(
        model,
        prior,
        prior_test,
        nb_simul,
      ...
      Lenormand = .ABC_Lenormand_cluster(
        model,
        prior,
        prior_test,
        nb_simul,
...
.ABC_MCMC2_cluster <-
  function(model,
    ...
    tab_param <- NULL
    # initial draw of a particle
    initial <- .ABC_rejection_internal_cluster(
      model,
      prior,
      prior_test,
      n_calibration,
    ...
    tab_simul_summary_stat <-
      as.matrix(initial[, (nparam + 1):(nparam + nstat)])
    sd_simul <- array(0, nstat)
    for (i in 1:nstat) {
      sd_simul[i] <- sd(tab_simul_summary_stat[, i])
    }
...
.ABC_MCMC3_cluster <-
  function(model,
    ...
    # initial draw of a particle
    initial <-
      .ABC_rejection_internal_cluster(model, prior, prior_test,
        nb_simul = n_calibration,
